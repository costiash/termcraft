...
|---|---|---|
| **1. Actual agent behavior** | **Smoke CLOSED (0.7.6) and create CLOSED (0.7.10) in CI with sandboxed Bash/Write/Edit; audit still the owner's live run** | An `evals/` suite for `claude plugin eval` now encodes the three workflows (theme pick; harness create on a scaffolded `install.sh` fixture; harness audit of a wizard with planted defects) with trace/tool/file/LLM graders. The smoke case was executed live during the 0.7.5 review (result recorded below). The create and audit cases run real agents for many turns with `--scaffold --allow-tools Bash Write Edit`; they were parsed and loaded by the CLI but not executed here — run them and keep `evals/results/` transcripts. |
...
## Live create in CI — 0.7.10

`workflow_dispatch` with `cases=create` (`--scaffold --allow-tools Bash Write Edit`, bubblewrap, PyPI blocked) on plugin 0.7.9: **score 1.0, 7/7 graders, 16 turns, 1228 s, $6.80**. The architect digested the fixture, wrote a 9-row process model, generated an 8-stage `relayctl.py` on the unmodified kit at the stdlib tier, delegated the look to the designer (noir, ASCII banner twin — the 0.7.9 fix visible live), wrote 13 pty scenarios and drove them to 13/13 (inert read-only modes proven by tree snapshots, no secret in journal or transcript, exit 3 vs 4 checked against the runtime), ran the harness for real in the tree and restored it, and handed back a one-screen summary that disclosed the two things it could not verify (no package index; the fixture lacked the `relay` module its `install.sh` promised — a real fixture defect, fixed). The first attempt on 0.7.8 completed the same work but timed out at 1800 s before the hand-back; budgets are now 3600 s / 150 turns. Evidence: `evals/evidence/2026-09-17-ci-create-0.7.8.json`, `2026-09-18-ci-create-0.7.9.json`.
